...
for version in "${versions[@]}"; do
	...
	if [ -d "$version/phusion" ]; then
		{ generated_warning; cat Dockerfile-phusion.template; } > "$version/phusion/Dockerfile"
		cp -v \
			docker-php-entrypoint \
			docker-php-ext-* \
			docker-php-source \
			"$version/phusion/"
		dockerfiles+=( "$version/phusion/Dockerfile" )
	fi
	...
	; do
		...
		if [ "$target" != "$variant" ]; then
			...
			echo " **** Variant Variant: $variantVariant"
